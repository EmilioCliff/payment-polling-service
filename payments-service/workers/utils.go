...
func (processor *RedisTaskProcessor) createTransaction(ctx context.Context, data createTransactionData) error {
	_, err := processor.store.CreateTransaction(ctx, db.CreateTransactionParams{
		TransactionID:      data.TransactionID,
		PaydTransactionRef: data.PaydTransactionRef,
		UserID:             data.UserID,
		Action:             data.Action,
